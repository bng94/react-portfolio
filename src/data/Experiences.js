export const ExperiencesData = [
  {
    title: "Front End Web Developer",
    company: "Envogue International",
    url: "https://madeyn.com/",
    dateRange: "Nov 2021 - Present",
    description: [
      "Designing and developing full functionality from scratch with the development team to maintain and upgrading the new startup company madeyn with technologies such as React, GraphQL and AWS.",
      "Coordinating and Collaborating with the whole team and project stakeholders to identify product and technical requirements to ensure app is ready for production.",
      "Collaborated with the team to build the tunnel between EC2 and Github for code deployment for production phase.",
      "Provides technical leadership throughout the Agile design process and guidance with regards to practices, procedures and techniques.",
    ],
  },
  {
    title: "Software Engineer M.S.",
    company: "Pace University",
    url: "https://www.pace.edu/",
    dateRange: "Jan 2019 - Aug 2020",
    description: [
      "Graduated with a GPA of 3.95",
      "Acquired knowledge of Software Development Life Cycle and Project Management for the start of a project to end of a project life cycle",
      "Boosted my knowledge of Mobile and Web Development Standards",
    ],
  },
  {
    title: "IT Tech Support",
    company: "RF CUNY",
    url: "https://www.rfcuny.org",
    dateRange: "Oct 2016 - Dec 2017",
    description: [
      "Maintained computer software applications in use of classrooms and offices to ensure smooth performance for all educators and students.",
      "Oversaw the use of technology equipment by teaching and providing assistances to staff and students to use the computers and tablets.",
      "Assessed, diagnosed, and solved hardware and software problems throughout the school",
    ],
  },
  {
    title: "Computer Science B.S.",
    company: "Brooklyn College",
    url: "https://www.brooklyn.cuny.edu/",
    dateRange: "Dec 2016",
    description: [
      "Learned how to create mobile apps for Google Play Store.",
      "Learned algorithms for faster solutions to problem-solving operations",
      "Acquired knowledges of best way to efficiently organizie data in the code",
    ],
  },
];
